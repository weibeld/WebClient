...
export async function decryptPrivateKey(privKey, privKeyPassCode) {
    console.log(`[Call] pmcrypto.decryptPrivateKey()
privKey: ${privKey}
privKeyPassCode: ${privKeyPassCode}`);
    ...
    // Convert armored key string into internal OpenPGP private key object
    const [key] = await getKeys(privKey);
    // Decrypt OpenPGP private key object
    ...
    console.log(`[Executing] pmcrypto.decryptPrivateKey()
???? Decrypted private key:
${key.armor()}`);;
    console.log(`[Return] pmcrypto.decryptPrivateKey()`);//\n${JSON.stringify(key, null, 2)}`);
    return key;
}
