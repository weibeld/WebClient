...
export async function generateKey({ passphrase, date = serverTime(), ...rest }) {
    console.log(`[Call] pmcrypto.generateKey()
{
  ...
  rest: ${JSON.stringify(rest)}
    ...
    const key = await openpgp.generateKey({ passphrase, date, ...rest });
    console.log(`[Executing] pmcrypto.generateKey()
privateKeyArmored:
${key.privateKeyArmored}
publicKeyArmored:
${key.publicKeyArmored}`);
    console.log(`[Return] pmcrypto.generateKey()`);
...
// Convert a key (in binary or armored ASCII format) into an OpenPGP key object
export async function getKeys(rawKeys = '') {
    console.log(`[Call] pmcrypto.getKeys()`);
    const method = rawKeys instanceof Uint8Array ? 'read' : 'readArmored'; // openpgp.key.read or openpgp.key.readArmored
    // Calls openpgp.key.read() or openpgp.key.readArmored()
    ...
    console.log(`[Return] pmcrypto.getKeys()`);
    return keys.keys;
}
