...
export default async function encryptMessage({ armor = true, ...options }) {
    console.log(`[Call] pmcrypto.encryptMessage()
data: ${options.data}
Number of public keys: ${options.publicKeys.length}
Public keys:
${options.publicKeys.map(pk => pk.armor())}`);
    ...
    if (options.detached) {
        ...
        options.privateKeys = options.privateKeys ? [].concat(options.privateKeys) : options.privateKeys;
    ...
    const result = await openpgp.encrypt(options);
    console.log(`[Return] pmcrypto.encryptMessage()\n${result.data}`);
